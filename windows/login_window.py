...
class LoginWindow(QWidget):
    ...
    def contact_admin(self):
        ...
        label = QLabel("Please describe your issue:")
        layout.addWidget(label)

        self.admin_message_text = QTextEdit()
        self.admin_message_text.setMinimumHeight(200)
        layout.addWidget(self.admin_message_text)

        button_layout = QHBoxLayout()
        
        send_btn = QPushButton("Send")
        send_btn.clicked.connect(lambda: self._send_admin_message(dialog))
    ...
    def _send_admin_message(self, dialog):
    ...
    def attempt_login(self):
        ...
        try:
            success, auth_username, auth_role = self.manager.authenticate(username, password)
            if success:
                # Optionally remember the account
                if self.remember_checkbox.isChecked():
                    self.manager.remember_device(username)
                
                self.login_success_callback(auth_username, auth_role)
            else:
    ...
    def check_device_login(self):
        ...
        try:
            username, role = self.manager.check_device_login()
            if username:
                # Auto-login the user
